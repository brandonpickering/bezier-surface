...
static void render_ad(const vec3f *ctrls, bool wireframe, float tolerance,
                                              float u0, float v0,
                                              float u1, float v1,
                                              float u2, float v2) {
  ...
  if (ngood == 2) {
    ...
    render_ad(ctrls, wireframe, tolerance,
                                cs[i][0], cs[i][1],
...
void render_ad(const std::vector<vec3f> &patch, bool wireframe,
                float tolerance) {
  render_ad(patch.data(), wireframe, tolerance, 0,0, 1,0, 0,1);
  render_ad(patch.data(), wireframe, tolerance, 1,0, 0,1, 1,1);
}
...
void render_object(const std::vector<std::vector<vec3f>> &patches,
  ...
  for (const std::vector<vec3f> &patch : patches) {
    if (adaptive)
      ...
      render_u(patch, wireframe, parameter);
  }
}
